...
# Título da aplicação
st.title("Cadastro - Virtual Games Store")

# Formulário para cadastro do produto
product_name = st.text_input("Nome do Jogo")
description = st.text_area("Descrição do Jogo")
price = st.number_input("Preço do Jogo", min_value=0.0, format="%.2f")
uploaded_file = st.file_uploader("Imagem do Jogo", type=["png", "jpg", "jpeg"])
...
def list_products_sql():
    try:
        ...
        st.error(f"Erro ao listar jogos: {e}")
        return []

# Função para exibir a lista de produtos na tela   
def list_jogos_screen():
...
if st.button("Cadastrar Jogo"):
    if not product_name or not description or price is None:
        ...
        if insert_product_sql(product_data):
            st.success("Jogo cadastrado com sucesso no Azure SQL!")
            list_jogos_screen()
        else:
            st.error("Houve um problema ao cadastrar o jogo no Azure SQL.")
...
st.header("Listagem dos Jogos")


# Botão para carregar e exibir a lista de jogos
if st.button("Listar Jogos"):
    list_jogos_screen()